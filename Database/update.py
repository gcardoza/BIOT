...
def process_IrrigationStatus(node_Data):
    ...
    # ***** SEND ZONE STATUS TO THE HOME ASSISTANT LISTENING MQTT TOPIC *****
        ...
        if(activeZone == "0") : 
            print("    -> All zones are Off")
            client.publish("/BioT/IrrigationStatus/Z1", zoneStatus)
            client.publish("/BioT/IrrigationStatus/Z2", zoneStatus)
            client.publish("/BioT/IrrigationStatus/Z3", zoneStatus)
            client.publish("/BioT/IrrigationStatus/Z4", zoneStatus)        
            client.publish("/BioT/IrrigationStatus/Z5", zoneStatus)
            client.publish("/BioT/IrrigationStatus/Z6", zoneStatus)
            client.publish("/BioT/IrrigationStatus/Z7", zoneStatus)
            client.publish("/BioT/IrrigationStatus/Z8", zoneStatus)
        elif(activeZone == "1") : client.publish("/BioT/IrrigationStatus/Z1", zoneStatus)
        elif(activeZone == "2") : client.publish("/BioT/IrrigationStatus/Z2", zoneStatus)
        elif(activeZone == "3") : client.publish("/BioT/IrrigationStatus/Z3", zoneStatus)
        elif(activeZone == "4") : client.publish("/BioT/IrrigationStatus/Z4", zoneStatus)        
        elif(activeZone == "5") : client.publish("/BioT/IrrigationStatus/Z5", zoneStatus)
        elif(activeZone == "6") : client.publish("/BioT/IrrigationStatus/Z6", zoneStatus)
        elif(activeZone == "7") : client.publish("/BioT/IrrigationStatus/Z7", zoneStatus)
